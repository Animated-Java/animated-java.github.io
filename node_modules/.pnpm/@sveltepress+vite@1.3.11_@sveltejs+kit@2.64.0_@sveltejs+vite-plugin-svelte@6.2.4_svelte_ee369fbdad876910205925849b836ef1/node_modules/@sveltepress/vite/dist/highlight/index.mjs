const COMMAND_RE = /\/\/ \[svp! ((hl)|(~~)|(\+\+)|(--)|(df)|(fc)|(!!))(:\S+)?\]/;
const highlightLine = (linesNumberToHighlight, idx, lines) => {
...
const focus = (linesNumberToFocus, idx, lines) => {
  const num = Number(linesNumberToFocus);
  const wrapFocus = (top, height) => `<div class="svp-code-block--focus" style="top: ${top};height: ${height};"></div>`;
...
const processCommands = (line, lineIndex, lineLength) => {
  const commandDoms = [];
  let newLine = line;
...
function prepareCodeBlock(code, meta) {
...
function wrapCodeBlock(highlightedHtml, lang, prepared, options = {}) {
  ...
  const titleHtml = title ? `<div class="svp-code-block--title">${title}</div>
` : "";
  const lineNumbersHtml = containLineNumbers ? `<div class="svp-code-block--line-numbers">${lines.map((_, i) => `<div class="svp-code-block--line-number-item">${i + 1}</div>`).join("\n")}</div>` : "";
...
export { COMMAND_CHEAT_LIST, COMMAND_RE, prepareCodeBlock, processCommands, wrapCodeBlock };
