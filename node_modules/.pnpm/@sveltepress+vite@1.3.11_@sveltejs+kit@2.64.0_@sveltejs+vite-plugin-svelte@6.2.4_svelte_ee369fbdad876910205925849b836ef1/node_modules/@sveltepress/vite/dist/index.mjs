...
function applyRemarkPluginsBeforeRehype(remarkPlugins) {
  ...
  processorAfterRemarkParse.use(emoji).use(remarkDirective).use(reserveSvelteCommands$1).use(reserveSvelteCommands).use(enhancedImagesPlugin).use(remarkFrontmatter).use(remarkExtractFrontmatter, { yaml: parse }).use(remarkGfm);
...
function deriveRoutePath(filePath, routesDir) {
...
function sectionOf(routePath) {
  if (routePath === "/")
    return "";
  const parts = routePath.split("/").filter(Boolean);
  return parts[0] || "";
}
function generateLlmsTxt(config, siteConfig) {
...
function isRemarkPluginsOrderer(value) {
  return typeof value === "function";
}
function isRehypePluginsOrderer(value) {
  return typeof value === "function";
}
